...
#include "evenc_shengtai_macro_ov02b10mipiraw_Sensor.h"
...
#define PFX "EVENC_SHENGTAI_MACRO_OV02B10_camera_sensor"
...
#define DEVICE_VERSION_EVENC_SHENGTAI_MACRO_OV02B10  "evenc_shengtai_macro_ov02b10"
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = EVENC_SHENGTAI_MACRO_OV02B10_SENSOR_ID,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
            ...
            if (*sensor_id == 0x2c) {
				...
				if(module_id == 0x07){//shengtai
							*sensor_id = imgsensor_info.sensor_id;
							LOG_INF("evenc_shengtai_macro_ov02b10 read[%s] sensor id: 0x%x\n",__func__, *sensor_id);
							return ERROR_NONE;
				}
            }
            retry--;
    ...
    if (*sensor_id != imgsensor_info.sensor_id) {
        LOG_INF("EVENC_SHENGTAI_MACRO_OV02B10 get_imgsensor_id failed: 0x%x\n", *sensor_id);
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            if (sensor_id == 0x2c) {
                sensor_id = imgsensor_info.sensor_id;
                LOG_INF("EVENC_SHENGTAI_MACRO_OV02B10 open success: 0x%x\n", sensor_id);
                break;
            }
...
{
     pr_info("[evenc_shengtai_macro_ov02b10] preview mode start\n");
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
          MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
     pr_info("[evenc_shengtai_macro_ov02b10] capture mode start\n");
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
          MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
     pr_info("[evenc_shengtai_macro_ov02b10] normal_video mode start\n");
...
static kal_uint32 hs_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
     MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
     pr_info("[evenc_shengtai_macro_ov02b10] hs_video mode start\n");
...
static kal_uint32 slim_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
     MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
     pr_info("[evenc_shengtai_macro_ov02b10] slim_video mode start\n");
...
static kal_uint32 custom1(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
    MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
     pr_info("[evenc_shengtai_macro_ov02b10] custom1 mode start\n");
...
UINT32 EVENC_SHENGTAI_MACRO_OV02B10_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
