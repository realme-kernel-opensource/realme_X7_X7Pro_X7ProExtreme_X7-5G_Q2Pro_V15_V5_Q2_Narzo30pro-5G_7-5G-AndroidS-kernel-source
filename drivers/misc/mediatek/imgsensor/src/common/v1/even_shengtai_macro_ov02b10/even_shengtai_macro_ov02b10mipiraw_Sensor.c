...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = EVEN_SHENGTAI_MACRO_OV02B10_SENSOR_ID,
    ...
    .pre = {
            ...
            .framelength = 1237,            //record different mode's framelength
...
// {
    ...
    // iReadRegI2C(pusendcmd , 2, (u8*)&get_byte,1,0xA4/*EEPROM_READ_ID*/);
    // if (get_byte == 0) {
        // iReadRegI2C(pusendcmd, 2, (u8 *)&get_byte, 1, 0xA4/*EEPROM_READ_ID*/);
    // }
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    if((gain >= 0x40) && (gain <= (15*0x40))) //base gain = 0x40
    {
        ...
        if(iReg<=0x10)
        {
            ...
            write_cmos_sensor(0xfe, 0x02);//fresh
            LOG_INF("EVEN_SHENGTAI_MACRO_OV02B10MIPI_SetGain = 16");
        }
        else if(iReg>= 0xf8)//gpw
        {
            ...
            write_cmos_sensor(0xfe, 0x02);//fresh
            LOG_INF("EVEN_SHENGTAI_MACRO_OV02B10MIPI_SetGain = 160");
        }
        else
        {
            write_cmos_sensor(0xfd, 0x01);
            ...
            write_cmos_sensor(0xfe, 0x02);//fresh
            LOG_INF("EVEN_SHENGTAI_MACRO_OV02B10MIPI_SetGain = %d",iReg);
        }
    }
    else
...
static kal_uint32 check_board(void) //even,evenb:1, evenc:2,
{
...
{
    kal_uint16 get_byte = 0;
    char pusendcmd[2] = {(char)(0x0000 >> 8), (char)(0x0000 & 0xFF)};

    iReadRegI2C(pusendcmd, 2, (u8 *)&get_byte, 1, 0xA4);/*EEPROM_READ_ID*/
    LOG_INF("the module id is %d\n", get_byte);
    return get_byte;
}
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
            LOG_INF("even_shengtai_macro_ov02b10 sensor_idx: %#x", *sensor_id);
            if (*sensor_id == 0x002C) {
                module_id = read_module_id();
                if(check_board() == 1) {
                    if(module_id == 0x07){//shengtai
                        *sensor_id = imgsensor_info.sensor_id;
                        LOG_INF("even_shengtai_macro_ov02b10 read[%s] sensor id: 0x%x\n",__func__, *sensor_id);
                        return ERROR_NONE;
                    }
                } else {
    ...
    if (*sensor_id != imgsensor_info.sensor_id) {
        LOG_INF("EVEN_SHENGTAI_MACRO_OV02B10 get_imgsensor_id failed: 0x%x\n", *sensor_id);
        *sensor_id = 0xFFFFFFFF;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            if (sensor_id == 0x002C) {
                sensor_id = imgsensor_info.sensor_id;
                LOG_INF("EVEN_SHENGTAI_MACRO_OV02B10 open success: 0x%x\n", sensor_id);
                break;
            }
...
{
     pr_info("[even_shengtai_macro_ov02b10] preview mode start\n");
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
          MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
     pr_info("[even_shengtai_macro_ov02b10] capture mode start\n");
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
          MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
     pr_info("[even_shengtai_macro_ov02b10] normal_video mode start\n");
...
static kal_uint32 hs_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
     MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
     pr_info("[even_shengtai_macro_ov02b10] hs_video mode start\n");
...
static kal_uint32 slim_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
     MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
     pr_info("[even_shengtai_macro_ov02b10] slim_video mode start\n");
...
static kal_uint32 custom1(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
    MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
     pr_info("[even_shengtai_macro_ov02b10] custom1 mode start\n");
...
UINT32 EVEN_SHENGTAI_MACRO_OV02B10_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
