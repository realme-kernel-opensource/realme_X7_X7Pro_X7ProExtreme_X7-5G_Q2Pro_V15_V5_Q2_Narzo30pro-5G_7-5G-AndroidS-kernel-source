...
#include "ov02b1bmipimono_Sensor.h"
...
#define DEVICE_VERSION_OV02B1B "ov02b1b"
...
static  struct imgsensor_info_struct imgsensor_info = {
	...
	.sensor_id = OV02B1B_SENSOR_ID_APOLLOF,
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			*sensor_id = ((read_cmos_sensor(0x02) << 8) | read_cmos_sensor(0x03));
			printk("chenshipei get_imgsensor_id:0x%x  i2c:0x%x\n", *sensor_id, imgsensor.i2c_write_id);
			if (*sensor_id == 0x002B && read_cmos_eeprom_16(0x06) != 0x99) {
				*sensor_id = imgsensor_info.sensor_id;
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
                /*Readout SN data*/
                data_index = DUALCAM_CALI_DATA_LENGTH - 9;
                for (j = 0; j < CAMERA_MODULE_SN_LENGTH; j++) {
                    EepData[data_index+j] = '0';
                }
				/*dengchao@camera.drv add for register device info*/
				if (deviceInfo_register_value == 0) {
					register_imgsensor_deviceinfo("Cam_r2", DEVICE_VERSION_OV02B1B, 7);/*7 means shine*/
