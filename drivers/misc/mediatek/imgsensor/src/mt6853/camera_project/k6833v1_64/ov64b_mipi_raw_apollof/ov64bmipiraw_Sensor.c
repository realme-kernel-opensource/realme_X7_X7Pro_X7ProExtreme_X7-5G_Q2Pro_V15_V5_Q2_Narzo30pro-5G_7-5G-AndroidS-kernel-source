...
#define DEVICE_VERSION_OV64B "ov64b"
static kal_uint8 deviceInfo_register_value = 0;
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV64B_SENSOR_ID_APOLLOF,
    ...
    .custom4_delay_frame = 1,        //enter custom4 delay frame num
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {
    ...
    {9280, 6944, 16, 864, 9248, 5216, 4624, 2608, 0, 4, 4624, 2600, 0, 0, 4624, 2600}, /*custom2*/
...
/*dengchao@CAMERA.DRV. add for reading module id.*/
...
static void read_EepromQSC(void)
{
    ...
    gImgEepromInfo_21625.CamNormdata[0][0] = Eeprom_1ByteDataRead(0x00, 0xA0);
    gImgEepromInfo_21625.CamNormdata[0][1] = Eeprom_1ByteDataRead(0x01, 0xA0);
    imgsensor_info.module_id = Eeprom_1ByteDataRead(0x00, 0xA0);
    Oplusimgsensor_Registdeviceinfo(gImgEepromInfo_21625.pCamModuleInfo[0].name,
                                    gImgEepromInfo_21625.pCamModuleInfo[0].version,
                                    imgsensor_info.module_id);
    for(i = 2; i < 8; i++) {
       gImgEepromInfo_21625.CamNormdata[0][i] = Eeprom_1ByteDataRead(0x04+i, 0xA0);
    }
    for (i = 0; i < OPLUS_CAMERASN_LENS; i ++) {
       gImgEepromInfo_21625.CamNormdata[0][8+i] = Eeprom_1ByteDataRead(0xB0+i, 0xA0);
    }
    ...
    memcpy(&gImgEepromInfo_21625.CamNormdata[2][28], &Dac_master, 4);
    /*Read stereo eeprom data*/
    for (i = 0; i < CALI_DATA_MASTER_LENGTH_21625; i ++) {
       gImgEepromInfo_21625.stereoMWdata[i] = Eeprom_1ByteDataRead(OV64B_STEREO_START_ADDR_21625+i, 0xA0);
    }
    gImgEepromInfo_21625.i4CurSensorIdx = 0;
    gImgEepromInfo_21625.i4CurSensorId = imgsensor_info.sensor_id;
#endif
}
        ...
        *sensor_id = return_sensor_id();
        if (*sensor_id == 0x5664) {
            *sensor_id = imgsensor_info.sensor_id;
            ...
            /*Readout SN data*/
            Eeprom_DataRead(&EepData[data_index],0xB0, CAMERA_MODULE_SN_LENGTH, EEPROM_ADDR);
            //dengchao@CAMERA.DRIVER.2020/9/9 add for reading module info
            data_index += CAMERA_MODULE_SN_LENGTH;
            LOG_INF("module info data_index is %d", data_index);
            ...
            //dengchao@CAMERA.DRV.2020/09/09,Add for register device info
            module_id = read_module_id();
            if (deviceInfo_register_value == 0) {
                register_imgsensor_deviceinfo("Cam_r1", DEVICE_VERSION_OV64B, module_id);
