...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KGM1ST_SENSOR_ID_20611,
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0000) << 8)
			...
			pr_info(
				"s5kgm1st_mipi_raw_20611: read_0x0000=0x%x, 0x0001=0x%x,0x0000_0001=0x%x *sensor_id:0x%x\n",
				read_cmos_sensor_8(0x0000),
				read_cmos_sensor_8(0x0001),
				read_cmos_sensor(0x0000), *sensor_id);
			if (*sensor_id == S5KGM1ST_SENSOR_ID) {
				*sensor_id = imgsensor_info.sensor_id;
				pr_info("s5kgm1st_mipi_raw_20611: i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}

			pr_info("s5kgm1st_mipi_raw_20611: Read sensor id fail, id: 0x%x *sensor_id:0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			pr_info("s5kgm1st_mipi_raw_20611: Read sensor id fail, write id: 0x%x, sensor_id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
