...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = HI846_SENSOR_ID_20611,
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			do {
				*sensor_id =return_sensor_id();
				pr_info("hi846_mipi_raw_20611: hi846 [get_imgsensor_id] sensor_id = 0x%x",*sensor_id);
				if (*sensor_id == HI846_SENSOR_ID) {
					*sensor_id = imgsensor_info.sensor_id;
					pr_info("hi846_mipi_raw_20611: i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
					return ERROR_NONE;
				}
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			pr_info("hi846_mipi_raw_20611: hi846 [open]: sensor_id = 0x%x\n",sensor_id);
			if (sensor_id == HI846_SENSOR_ID) {
				sensor_id = imgsensor_info.sensor_id;
				LOG_INF("hi846_mipi_raw_20611: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
				break;
			}
			pr_info("hi846_mipi_raw_20611: open:Read sensor id fail open i2c write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id) {
