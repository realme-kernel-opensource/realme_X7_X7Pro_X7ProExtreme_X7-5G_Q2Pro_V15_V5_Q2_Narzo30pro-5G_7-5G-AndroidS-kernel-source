...
static struct imgsensor_info_struct imgsensor_info = {
		.sensor_id = S5K3P9SP_SENSOR_ID_20611 ,
...
/*
static kal_uint16 addr_data_pair_normal_video_for_18561[] = {
	0x6028, 0x4000,
	0x6214, 0x7970,
	0x6218, 0x7150,
	0x6028, 0x2000,
...
static void sensor_init(void)
{
	...
	iWriteRegI2CTiming((u8*)uTnpArrayInit , (u16)sizeof(uTnpArrayInit), imgsensor.i2c_write_id, imgsensor_info.i2c_speed);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_16_8(0x0000) << 8) | read_cmos_sensor_16_8(0x0001));
			pr_info("s5k3p9sp_mipi_raw_20611: read out sensor id 0x%x \n", *sensor_id);
			if (*sensor_id == S5K3P9SP_SENSOR_ID) {
				*sensor_id = imgsensor_info.sensor_id;
				pr_info("s5k3p9sp_mipi_raw_20611: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				#ifdef OPLUS_FEATURE_CAMERA_COMMON
				read_4cell_from_eeprom_s5k3p9sp();
				#endif
				return ERROR_NONE;
			}
			pr_info("s5k3p9sp_mipi_raw_20611: Read sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
						...
						MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	LOG_INF("Warning: current_fps %d fps is not support, so use custom2's setting: %d fps!\n", imgsensor.current_fps, imgsensor_info.custom2.max_framerate / 10);
						...
						MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	LOG_INF("Warning: current_fps %d fps is not support, so use custom3's setting: %d fps!\n", imgsensor.current_fps, imgsensor_info.custom3.max_framerate / 10);
...
UINT32 S5K3P9SP_MIPI_RAW_20611_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
