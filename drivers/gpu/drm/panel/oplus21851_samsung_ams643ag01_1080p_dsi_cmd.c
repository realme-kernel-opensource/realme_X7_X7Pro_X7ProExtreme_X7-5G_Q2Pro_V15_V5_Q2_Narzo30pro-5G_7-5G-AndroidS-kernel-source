...
#include "../mediatek/mtk_corner_pattern/oplus21851_AMS643AG01_data_hw_roundedpattern.h"
#endif
...
static int lcm_panel_poweron(struct drm_panel *panel)
{
	...
	pmic_ldo_set_voltage_mv(5, 1500);//fan53870 ldo5 1.5v
	msleep(5);
...
static int lcm_panel_poweroff(struct drm_panel *panel)
{
	...
	pmic_ldo_set_disable(5);/*fan53870 disable ldo5*/
	msleep(5);
	ctx->bias_gpio = devm_gpiod_get(ctx->dev, "bias", GPIOD_OUT_HIGH);
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	pmic_ldo_set_voltage_mv(5, 1500);
	msleep(2);
	ctx->bias_gpio = devm_gpiod_get(ctx->dev, "bias", GPIOD_OUT_HIGH);
	gpiod_set_value(ctx->bias_gpio, 1);
	devm_gpiod_put(ctx->dev, ctx->bias_gpio);
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "oplus21851_samsung_ams643ag01_1080p_dsi_cmd,lcm", },
	{ }
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "oplus21851_samsung_ams643ag01_1080p_dsi_cmd,lcm",
		.owner = THIS_MODULE,
