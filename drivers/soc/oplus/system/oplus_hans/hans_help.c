			  ...
			  struct binder_transaction_data *tr)
{
	...
	if ((tr->flags & TF_ONE_WAY) /*report async binder call*/
		...
		if (!copy_from_user(buf_data, (char*)tr->data.ptr.buffer, buf_data_size)) {
			...
			hans_report(ASYNC_BINDER, task_tgid_nr(proc->tsk), task_uid(proc->tsk).val,
			  ...
			  int is_async)
{
	...
	if (is_async
		...
		if (p != NULL && is_frozen_tg(p)) {
			hans_report(ASYNC_BINDER, task_tgid_nr(current), task_uid(current).val,
...
{
	if ((is_frozen_tg(dst) || is_zombie_tg(dst))  /*signal receiver thread group is frozen?*/
		&& (sig == SIGKILL || sig == SIGTERM || sig == SIGABRT || sig == SIGQUIT
		    || sig == SIGSEGV || sig == SIGIO)) {
	...
	if ((is_belong_cpugrp(dst) || is_zombie_tg(dst))  /*signal receiver thread group is cpuctl?*/
			&& (sig == SIGKILL || sig == SIGTERM || sig == SIGABRT || sig == SIGQUIT)) {
		if (hans_report(SIGNAL, task_tgid_nr(killer), task_uid(killer).val,
				task_tgid_nr(dst), task_uid(dst).val, "signal", -1) == HANS_ERROR) {
			printk(KERN_ERR
			       "HANS: report signal-cpuctl failed, sig = %d, caller = %d, target_uid = %d\n",
			       sig, task_tgid_nr(killer), task_uid(dst).val);
		}
	}
...
void hans_check_uid_proc_status_debug(struct binder_proc *proc,
		...
		if (thread->task != NULL) {
				...
				list_for_each_entry(w, &thread->todo, entry) {
					if (w != NULL && w->type == BINDER_WORK_TRANSACTION) {
						...
						if (btrans != NULL && btrans->to_thread == thread) {
							...
							hans_report(type, from_pid, from_uid, to_pid, to_uid, "FROZEN_TRANS_THREAD",
								    need_reply);
			...
			if (btrans) {
				...
				if (btrans->to_thread == thread) {
					...
					get_uid_pid(&from_pid, &from_uid, &to_pid, &to_uid, btrans);
					spin_unlock(&btrans->lock);
					hans_report(type, from_pid, from_uid, to_pid, to_uid, "FROZEN_TRANS_STACK",
						    need_reply);
		...
		list_for_each_entry(w, &proc->todo, entry) {
			if (w != NULL && w->type == BINDER_WORK_TRANSACTION) {
				...
				if (btrans != NULL && btrans->to_thread == thread) {
					...
					get_uid_pid(&from_pid, &from_uid, &to_pid, &to_uid, btrans);
					spin_unlock(&btrans->lock);
					hans_report(type, from_pid, from_uid, to_pid, to_uid, "FROZEN_TRANS_PROC",
						    need_reply);
