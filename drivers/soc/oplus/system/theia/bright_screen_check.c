...
#if IS_ENABLED(CONFIG_OPLUS_MTK_DRM_GKI_NOTIFY)
	if (g_bright_data.blank == MTK_DISP_BLANK_UNBLANK) {
#elif IS_ENABLED (CONFIG_DRM_MSM)
	if (g_bright_data.blank == MSM_DRM_BLANK_UNBLANK) {
#else
	if (g_bright_data.blank == FB_BLANK_UNBLANK) {
#endif
...
{
	return (BRIGHT_STATUS_CHECK_ENABLE == get_status()
		|| BRIGHT_STATUS_CHECK_DEBUG == get_status()) && g_bright_data.get_log;
}
...
static void get_brightscreen_check_dcs_logmap(char *logmap)
{
	...
	snprintf(logmap, 512,
		 ...
		 PWKKEY_BRIGHT_SCREEN_DCS_LOGTYPE,
		 g_bright_data.error_id, g_bright_data.error_count, get_systemserver_pid(),
		 stages, get_log_swich() ? "true" : "false");
}
...
static bool is_bright_last_stage_skip()
{
	...
	nLen = sizeof(bright_last_skip_block_stages) / sizeof(
		       bright_last_skip_block_stages[0]);
...
static bool is_bright_contain_skip_stage()
{
	...
	for (i = 0; i < nArrayLen; i++) {
		if (strstr(stages, bright_skip_stages[i]) != NULL) {
			BRIGHT_DEBUG_PRINTK("is_bright_contain_skip_stage return true, stages:%s",
					    stages);
			return true;
		}
	}
...
static void get_bright_resume_dcs_logmap(char *logmap)
{
	snprintf(logmap, 512,
		 ...
		 PWKKEY_BRIGHT_SCREEN_DCS_LOGTYPE,
		 g_bright_data.error_id, g_bright_data.error_count,
...
{
	struct bright_data *bri_data = container_of(work, struct bright_data,
				       error_happen_work);
...
static ssize_t bright_screen_cancel_proc_write(struct file *file,
		...
		size_t count, loff_t *off)
{
	...
	if (g_bright_data.status == BRIGHT_STATUS_INIT
...
static ssize_t bright_screen_cancel_proc_read(struct file *file,
		char __user *buf,
		size_t count, loff_t *off)
{
	return 0;
...
void bright_screen_check_init(void)
{
	...
	if (mtk_disp_notifier_register("oplus_theia", &g_bright_data.fb_notif)) {
		BRIGHT_DEBUG_PRINTK("mtk_disp_notifier_register failed!");
	}
#elif IS_ENABLED (CONFIG_DRM_MSM)
