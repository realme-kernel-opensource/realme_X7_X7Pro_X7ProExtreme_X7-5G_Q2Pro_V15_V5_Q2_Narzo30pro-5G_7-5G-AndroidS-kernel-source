...
int tfa_irq_clear(struct tfa_device *tfa, enum tfa9912_irq bit)
{
	...
	if (bit == tfa9912_irq_all) {
		...
		for (reg = TFA98XX_INTERRUPT_IN_REG1;
				reg < TFA98XX_INTERRUPT_IN_REG1 + 3;
				reg++)
...
void tfa_set_query_info(struct tfa_device *tfa)
{
	...
	/*TODO use the getfeatures() for retrieving*/
	/*the features [artf103523]	tfa->supportDrc = supportNotSet;*/
		...
		int features_from_cnt[3])
{
	...
	tfa98xx_convert_bytes2data(sizeof(bytes), bytes,
			&features_from_MTP[1]);
...
/****************** patching ***********************/
static enum Tfa98xx_Error
tfa98xx_process_patch_file(struct tfa_device *tfa,
		int length,
...
tfa98xx_wait_result(struct tfa_device *tfa,
...
enum Tfa98xx_Error tfa_dsp_msg_v6_write(struct tfa_device *tfa, int length,
		const char *buffer)
{
	int offset = 0;
...
enum Tfa98xx_Error tfa_dsp_msg_v6_write_v6_id(struct tfa_device *tfa,
				int length,
				const char *buffer,
				uint8_t cmdid[3])
{
...
enum Tfa98xx_Error tfa_dsp_msg_v6_read(struct tfa_device *tfa, int length,
		unsigned char *bytes)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
	...
	while (num_bytes > 0) {
		...
		error = tfa98xx_read_data(tfa,
				FAM_TFA98XX_CF_MEM, burst_size,
				bytes + offset);
		if (error != Tfa98xx_Error_Ok)
			return error;
		...
		const char *buf24)
{
	...
	if (tfa->ext_dsp == 1) {
		...
		/* if the buffer is full we need to send the */
		/* existing message and add the current message */
	...
	if (error != Tfa98xx_Error_Ok)
		error = (enum Tfa98xx_Error) (error +
					...
					const char *buf)
{
	...
	error = tfa_dsp_msg_v6_write(tfa, length, buf);
	if (error != Tfa98xx_Error_Ok)
		return error;
 ...
 */
enum Tfa98xx_Error tfa_dsp_msg_v6_id(struct tfa_device *tfa, int length,
			const char *buf, uint8_t cmdid[3])
{
	...
	/* write the message and notify the DSP */
	error = tfa_dsp_msg_v6_write_v6_id(tfa, length, buf, cmdid);
	if (error != Tfa98xx_Error_Ok)
		return error;
		...
		unsigned int start_offset, int num_words, int *pValues)
{
	...
	for (; num_bytes > 0;) {
		...
		error = tfa98xx_read_data(tfa,
			FAM_TFA98XX_CF_MEM, burst_size, bytes);
		if (error != Tfa98xx_Error_Ok)
			goto tfa98xx_dsp_read_mem_v6_exit;
...
/* wrapper for dsp_msg that adds opcode and*/
/* 3 bytes required for MBDrcDynamics */
...
enum Tfa98xx_Error
tfa98xx_dsp_write_preset_v6(struct tfa_device *tfa, int length,
...
enum Tfa98xx_Error
tfa98xx_select_mode(struct tfa_device *tfa,
			...
			const uint16_t bf, const uint16_t value)
{
	...
	/* Only write when the current register value*/
	/* is not the same as the new value */
...
uint16_t tfa_get_bf_v6_value(const uint16_t bf, const uint16_t reg_value)
{
	uint16_t msk, value;
				...
				int force, int profile)
{
	...
	pr_debug("Startup of device [%s] is a %sstart\n",
		tfaContDevice_v6Name(tfa->cnt, tfa->dev_idx),
		value ? "cold" : "warm");
			...
			int profile)
{
	...
	err = tfaContWriteFile_v6sProf(tfa, profile, 0);
	if (err) {
		pr_debug("[%s] tfaContWriteFile_v6sProf error = %d\n",
			__func__, err);
		return err;
	}
...
enum Tfa98xx_Error tfaRunStartDSP(struct tfa_device *tfa)
{
	...
	/* Clear count_boot, should be reset */
	/*to 0 before the DSP reset is released */
...
enum Tfa98xx_Error tfaRunStartup(struct tfa_device *tfa, int profile)
{
	...
	err = tfaContWriteRegsDev_v6(tfa); // write device register settings
	PRINT_ASSERT(err);
	...
	*calibrateDone = 0;
	...
	if (TFA_GET_BF(tfa, MTPOTC)) {
		...
		if (tries_mtp_busy < MTPBWAIT_TRIES) {
			/* Because of the msleep TFA98XX_API_WAIT_TRIES
	...
	} else if (tries == TFA98XX_API_WAIT_TRIES) {
						...
						int next_profile, int vstep)
{
	...
	pr_debug("Active_profile:%s, next_profile:%s\n",
			tfaContProfileName(tfa->cnt,
				tfa->dev_idx, active_profile),
			tfaContProfileName(tfa->cnt,
	...
	if (tfa->bus != 0)  { /* non i2c  */
		...
		if ((TFA_GET_BF(tfa, CFE) != 0) &&
			...
			err = tfaContWriteFilesVstep(tfa,
				next_profile, vstep);
			if (err != Tfa98xx_Error_Ok)
				goto error_exit;
		}
...
int tfa_reset(struct tfa_device *tfa)
{
	...
	if (tfa->tfa_family == 1) {
		...
		/* Probus devices needs extra protection to ensure */
		/* proper reset behavior, this step is val*/
		/* id only in state other than powerdown */
...
enum Tfa98xx_Error tfa_dsp_get_calibration_impedance(struct tfa_device *tfa)
{
	...
	if (tfa_dev_mtp_get(tfa, TFA_MTP_OTC)) {
		...
		if (tfa->is_probus_device && tfa->cnt->ndev > 1)
			...
			pr_err("tfa->cnt->ndev=%d\n",tfa->cnt->ndev);
		nr_bytes = spkr_count * 3;
		error = tfa_dsp_cmd_id_write_read(tfa, MODULE_SPEAKERBOOST,
		...
		if (error == Tfa98xx_Error_Ok) {
			...
			for (i = 0; i < spkr_count; i++) {
				// for probus devices, calibration values coming
				// from soft-dsp speakerboost, are ordered in a
				// different way. Re-align to standard
				// representation.
				cal_idx = i;
				/*xiexiaoping.wt 20210929 add  for read calibrition value */
#ifndef CONFIG_SND_TFA98XX_I2C_ADD_REVERSE	
    pr_info("CONFIG_SND_TFA98XX_I2C_ADD_REVERSE tfa_dsp_get_calibration_impedance dev_idx=%d \n",tfa->dev_idx);	
				if ((tfa->is_probus_device &&
					tfa->dev_idx >= 1)) {
					cal_idx = 0;
				}
#endif /* #ifdef CONFIG_SND_TFA98XX_I2C_ADD_REVERSE */
  pr_err("skip cal_idx for I2C_ADD_REVERSE !\n");				
	...
	if (error == Tfa98xx_Error_Ok) {
		...
		for (i = 0; i < spkr_count; i++) {
			...
			cal_idx = i;
				/*xiexiaoping.wt 20210929 add  for read calibrition value */			
#ifndef CONFIG_SND_TFA98XX_I2C_ADD_REVERSE	
    pr_info("CONFIG_SND_TFA98XX_I2C_ADD_REVERSE tfa_dsp_get_calibration_impedance_for_aging  dev_idx=%d \n",tfa->dev_idx);		
			if ((tfa->is_probus_device &&
				tfa->dev_idx >= 1)) {
				cal_idx = 0;
			}
...
enum tfa_error tfa_dev_set_state(struct tfa_device *tfa, enum tfa_state state)
{
	...
	case TFA_STATE_INIT_CF: /* coolflux HW access possible (~initcf) */
		...
		do {
			err = (enum tfa_error)tfa98xx_dsp_system_stable
					(tfa, &ready);
			if (err != tfa_error_ok)
				return err;
			if (ready)
...
{
	/* FIXME: this 72 interrupt*/
	/*does not excist for 9912 */
